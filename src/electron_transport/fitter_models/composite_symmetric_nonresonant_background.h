...
/**
 * \file composite_symmetric_nonresonant_background.h
 * \brief Fitting model for nonresonant tunneling (symmetric coupling) combined
 *    with background tunneling.
 *
 * \author Matthew G.\ Reuter
 * \date January 2015
 */

#ifndef __composite_symmetric_nonresonant_background_h__
#define __composite_symmetric_nonresonant_background_h__
 ...
 * \f]
 * where \f$g_-\f$ is a lower bound for the conductance. To make things
 * smoother for computation, we take
 * \f[
 ...
 * \f[
 * \hat{P}_\mathrm{NR\ast background}(g) = N \int\limits_0^g \mathrm{d}g' \hat{P}_\mathrm{NR}(g-g') \hat{P}_\mathrm{background}(g').
 * \f]
 ...
 * \endif
 */
class CompositeSymmetricNonresonantBackgroundFitModel : public FitModel<1>
{
protected:
	 ...
	 *    vector).
	 *
	 * `cepsilon`, `cgamma`, and `gminus` parameters are needed for this model.
	 * See \c FitModel::create_initial_guess for more information.
	 *
	 * \throw invalid_argument_exception if `cepsilon`, `cgamma`, and `gminus`
	 ...
	 *
	 * Function to be used in the GSL QAGS routine to evaluate \f[
	 * \int\limits_{0}^{g} \mathrm{d}g' \frac{\left[ 1+\mathrm{erf}\left( \frac{g'-g_-}{kg_-} \right) \right] \left(d\sqrt{1-g+g'}-c_\varepsilon\sqrt{g-g'}\right)}{g' (1-g+g')^{5/2}} \exp\left[ - \frac{\left( c_\varepsilon\sqrt{g-g'} - c_\Gamma\sqrt{1-g+g'} \right)^2}{2(1-g+g')} \right].
	 * \f]
	 *
	 * \param[in] gp The current value of \f$g'\f$.
	 * \param[in] params The fit parameters, assumed to be in vector<double>
...
public:
	...
	CompositeSymmetricNonresonantBackgroundFitModel() = delete;
	virtual ~CompositeSymmetricNonresonantBackgroundFitModel() = default;
	 ...
	 * follows.
	 * \f{eqnarray}
	 * \frac{\partial \hat{P}(g)}{\partial c_\varepsilon} & = & N \mathrm{int\_dp\_dcepsilon}(g), \\
	 * \frac{\partial \hat{P}(g)}{\partial c_\Gamma} & = & N \mathrm{int\_dp\_dcgamma}(g), \\
	 * \frac{\partial \hat{P}(g)}{\partial g_-} & = & \frac{-N}{g_- \sqrt{(g-g_-)(1-g+g_-)^3}} \exp\left[ -\frac{\left( c_\varepsilon\sqrt{g-g_-} - c_\Gamma\sqrt{1-g+g_-} \right)^2}{2(1-g+g_-)} \right], \\
	 * \frac{\partial \hat{P}(g)}{\partial N} & = & \mathrm{int\_p}(g).
	 * \f}
	 * where int_p is evaluated with \c CompositeSymmetricNonresonantBackgroundFitModel::int_p,
	 * and likewise for \c CompositeSymmetricNonresonantBackgroundFitModel::int_dp_dcepsilon and
	 * \c CompositeSymmetricNonresonantBackgroundFitModel::int_dp_dcgamma.
	 *
	 * \param[in] fitparam The fitting parameters.
	 * \param[in] x The independent variables for the function.
	 * \param[in] f The observed value of the fit function at x.
	 ...
	 * the integral in CompositeSymmetricNonresonantBackgroundFitModel::int_p.
	 *
	 * \param[in] fitparam The fitting parameters.
	 * \param[in] x The independent variables for the function.
	 * \param[in] f The observed value of the fit function at x.
