...
/**
 * \file tests/fit-CompositeSymNonresonantBackground.cc
 * \brief Test suite for the composite model (symmetric-coupling,
 *    nonresonant-tunneling and background) fit model.
 *
...
#include <electron_transport/fitter_models/composite_symmetric_nonresonant_background.h>

using namespace std;

/// Shortcut for the model type in this test.
using ModelType = molstat::transport::CompositeSymmetricNonresonantBackgroundFitModel;
...
int main(int argc, char **argv)
{
	...
	assert(abs(-0.536414 - returnval.second[ModelType::CEPSILON]) / 0.536414 < thresh);
	assert(abs(9.27073 - returnval.second[ModelType::CGAMMA]) / 9.27073 < thresh);
