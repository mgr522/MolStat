...
/**
 * \file tests/fit-CompositeInterferenceBackground.cc
 * \brief Test suite for the composite model (interference and background)
 *    fit model.
 *
 * \test Test suite for the composite model (interference and background)
 *    fit model.
 *
...
#include <electron_transport/fitter_models/composite_interference_background.h>

using namespace std;

/// Shortcut for the model type in this test.
using ModelType = molstat::transport::CompositeInterferenceBackgroundFitModel;
...
int main(int argc, char **argv)
{
	...
	assert(abs(-1.07601e-2 - returnval.second[ModelType::COMEGA]) / 1.07601e-2 < thresh);
	...
	assert(abs(-2.42792e-2 - returnval.second[ModelType::COMEGA]) / 2.42792e-2 < thresh);
	...
	assert(abs(-1.22860e-2 - returnval.second[ModelType::COMEGA]) / 1.22860e-2 < thresh);
